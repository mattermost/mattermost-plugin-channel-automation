...
| `ai_prompt`    | Sends a rendered prompt to an AI agent via the Mattermost AI Plugin. Requires `provider_type` and `provider_id`; can opt into the agent's stored system prompt with `use_agent_system_prompt`. |
