...
func TestChannelCreatedTrigger_CandidateAutomationIDs(t *testing.T) {
	tr := &trigger.ChannelCreatedTrigger{}

	t.Run("nil channel returns empty", func(t *testing.T) {
		ids, err := tr.CandidateAutomationIDs(&stubStore{}, &model.Event{Type: model.TriggerTypeChannelCreated})
		require.NoError(t, err)
		assert.Nil(t, ids)
	})
