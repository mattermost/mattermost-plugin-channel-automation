...
func TestUserJoinedTeamTrigger_CandidateFlowIDs(t *testing.T) {
	tr := &trigger.UserJoinedTeamTrigger{}

	t.Run("nil team returns empty", func(t *testing.T) {
		ids, err := tr.CandidateFlowIDs(&stubStore{}, &model.Event{Type: model.TriggerTypeUserJoinedTeam})
		require.NoError(t, err)
		assert.Nil(t, ids)
	})

	t.Run("delegates to store.GetFlowIDsForUserJoinedTeam", func(t *testing.T) {
