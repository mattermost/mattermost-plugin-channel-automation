...
func TestMembershipChangedTrigger_CandidateFlowIDs(t *testing.T) {
	tr := &trigger.MembershipChangedTrigger{}

	t.Run("nil channel returns empty", func(t *testing.T) {
		ids, err := tr.CandidateFlowIDs(&stubStore{}, &model.Event{Type: model.TriggerTypeMembershipChanged})
		require.NoError(t, err)
		assert.Nil(t, ids)
	})

	t.Run("delegates to store index", func(t *testing.T) {
		st := &stubStore{flowIDsByMembershipChannel: map[string][]string{"ch1": {"f1"}}}
