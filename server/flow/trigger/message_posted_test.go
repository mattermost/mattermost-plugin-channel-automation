...
func TestMessagePostedTrigger_CandidateFlowIDs(t *testing.T) {
	...
	t.Run("nil post returns empty", func(t *testing.T) {
		ids, err := tr.CandidateFlowIDs(&stubStore{}, &model.Event{Type: model.TriggerTypeMessagePosted})
		require.NoError(t, err)
		assert.Nil(t, ids)
	})

	t.Run("delegates to store.GetFlowIDsForChannel", func(t *testing.T) {
		st := &stubStore{flowIDsByChannel: map[string][]string{"ch1": {"f1", "f2"}}}
		event := &model.Event{
			Type: model.TriggerTypeMessagePosted,
			Post: &mmmodel.Post{ChannelId: "ch1"},
		}
