...
// not by matching events: Matches and CandidateFlowIDs are no-ops on the
...
// CandidateFlowIDs returns nil — schedule flows are not resolved via the
// event-matching path.
func (t *ScheduleTrigger) CandidateFlowIDs(_ model.Store, _ *model.Event) ([]string, error) {
	return nil, nil
}
