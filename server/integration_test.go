...
// deleteAutomation is a helper that deletes an automation and asserts success.
...
func TestIntegration(t *testing.T) {
	...
	t.Run("AutomationCRUDLifecycle", func(t *testing.T) {
		...
		require.Equal(t, http.StatusOK, resp.StatusCode)
		updatedAutomation := decodeAutomation(t, resp)
		assert.Equal(t, created.ID, updatedAutomation.ID)
		assert.Equal(t, "updated-automation", updatedAutomation.Name)
		assert.Equal(t, created.CreatedAt, updatedAutomation.CreatedAt, "CreatedAt must be immutable")
		assert.Equal(t, created.CreatedBy, updatedAutomation.CreatedBy, "CreatedBy must be immutable")
		assert.GreaterOrEqual(t, updatedAutomation.UpdatedAt, created.UpdatedAt)
		assert.Equal(t, "send-farewell", updatedAutomation.Actions[0].ID)
