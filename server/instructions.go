...
2. "ai_prompt": Runs an AI agent with a prompt and optional tools. With tools, the agent can perform actions (e.g. modify channels, manage members, search) — not just generate text. Does NOT post a message — chain a send_message action after to post the response.
   {"id": "ask", "ai_prompt": {"prompt": "...", "provider_type": "agent", "provider_id": "<agent-user-id>", "use_agent_system_prompt": true, "allowed_tools": ["<tool name from discovery>", "..."], "guardrails": {"channel_ids": ["<26-char channel id>", "..."]}, "request_as": "triggerer"}}
   {"id": "ask-with-extra", "ai_prompt": {"prompt": "...", "provider_type": "agent", "provider_id": "<agent-user-id>", "use_agent_system_prompt": true, "system_prompt": "Additional per-automation instructions", "allowed_tools": ["<tool name from discovery>", "..."]}}
   {"id": "ask-custom", "ai_prompt": {"prompt": "...", "provider_type": "agent", "provider_id": "<agent-user-id>", "system_prompt": "Custom instructions only for this automation", "allowed_tools": ["<tool name from discovery>", "..."]}}
   ...
   - use_agent_system_prompt (optional, agent only): set true when the selected agent's configured system prompt from the Agents screen should be applied at runtime. Prefer this over copying the agent's instructions into system_prompt so updates to the agent stay in sync.
   - system_prompt (optional): automation-specific system instructions for the AI. Omit when use_agent_system_prompt alone is enough; add "Additional per-automation instructions" when combining with use_agent_system_prompt; use without use_agent_system_prompt when the automation needs custom instructions only.
