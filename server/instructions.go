...
- They are a channel admin of the trigger channel or a team admin of that channel's team (for message_posted, schedule, membership_changed triggers in regular public/private channels).
...
How to check the channel role: call get_channel_info with the trigger channel_id. The response includes the requesting user's role in that channel as one of "admin", "member", "guest", or "not_member". "admin" satisfies the channel-admin requirement. Channel type "D" (DM) or "G" (GM) on the returned channel also satisfies the requirement regardless of role. For regular channels where the user is a non-admin member, a team admin of the channel's team is also permitted; if you cannot verify team admin from get_channel_info, proceed only if the user confirms they have team admin permission on that team. For team-scoped triggers (channel_created, user_joined_team) you cannot verify team admin from get_channel_info; tell the user that team admin permission on the target team is required and proceed only if they confirm they have it (or are a system admin).
