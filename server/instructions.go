...
2. "ai_prompt": Runs an AI agent with a prompt and optional tools. With tools, the agent can perform actions (e.g. modify channels, manage members, search) — not just generate text. Does NOT post a message — chain a send_message action after to post the response.
   {"id": "ask", "ai_prompt": {"prompt": "...", "provider_type": "agent", "provider_id": "<agent-user-id>", "use_agent_system_prompt": true, "system_prompt": "...", "allowed_tools": ["<tool name from discovery>", "..."], "guardrails": {"channel_ids": ["<26-char channel id>", "..."]}, "request_as": "triggerer"}}
   ...
   - system_prompt (optional): automation-specific system instructions for the AI. This can be combined with use_agent_system_prompt for per-automation additions.
